...
/// # Memory safety
///
/// A storage may not rely on this property for memory safety reasons. Instead, it may panic in
/// case this contract is violated.
...
/// # Associated `Key`
///
/// `SparseLinear` requires a `usize` key which  is expected to be linear; if this returns random /
...
pub trait SparseLinear: Id<Key = usize> {
    /// This is the bit set type that can be used by the storage. A storage which maps this ID
    ...
    /// **This does not guarantee validity and is equivalent to `as_key_unchecked`.**
...
/// # Generics
///
/// * `O`: "Original" ID; this might be `Self` if the ID is always valid, or the wrapped ID
///   (this is the case for `CheckedId`)
///
/// # Contract
///
...
pub trait ValidId<O: Id>: Id<Key = O::Key> {
    ...
    /// at the type-level. For a dynamic version see `Id::try_as_key`.
    fn as_key(&self) -> Cow<Self::Key>;
}

/// An ID that wraps another ID. It mirrors all basic ID property the wrapped ID has, through
/// respective blanket implementations.
///
/// # References
///
/// This is used for
