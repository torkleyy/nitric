...
impl DebugInstanceId {
    /// Creates a new, unique instance id that is only used with debug assertions enabled.
    pub fn new() -> Self {
        Default::default()
    }

    /// If debug assertions are enabled, asserts instance id and reference are equal.
    ...
    /// Creates a "reference" of this instance id. This is essentially like cloning, but
    /// `InstanceId`s don't implement `Clone` on purpose since values caring it should never be
    /// cloned.
    ///
...
impl Reference {
    ...
    /// Returns the `usize` representation of the referenced instance id which is used for all the
    /// assertions.
    #[inline]
    pub fn as_usize(&self) -> usize {
